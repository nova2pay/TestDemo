...
$.fn.optionTree = function(tree, options) {
    ...
    options = $.extend({
        choose: '请选择..', // string with text or function that will be passed current level and returns a string
    ...
    return this.each(function() {
        ...
        if (typeof tree === "object") { // many options exists for current nesting level
            ...
            var $select = $("<select>").css("margin-right","3px").attr(options.attr,name)
            .change(function() {
            ...
            $.each(tree, function(k, v) {
                ...
                if (options.indexed) {
                    ...
                    label = value = decodeURIComponent(k);
                }
            ...
            $select.attr("disabled",options.disabled);
            options.handler();
