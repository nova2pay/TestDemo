...
demo.add(1,0,"根目录","javascript:void(0)",null,true,true);
demo.add(2,1,"子目录","javascript:void(0)",null,true,true);
参数说明：当前节点id,父节点id，当前节点名称，点击事件，图标，是否勾选，是否可修改
...
function TreeView()
{
	...
	this.checkbox = false;
       // this.checked = false;
	this.Nodes ={ 0 : { ID : 0, ParentID : -1, Text : null, Href : null, Image : null, childNodes: new Array(), checkFlag: false, disnableFlag:false  , title: null} };
}
...
tv.add = function(iD,ParentiD,sText,sHref,sImage,sCheck,bDisnable,title) {
	this.Nodes[iD] = { ID : iD, ParentID : ParentiD, Text : sText, Href : sHref, Image : sImage , childNodes: new Array() , checkFlag : sCheck , disnableFlag: bDisnable , title: title};
...
tv.drawNode = function(ID) {
	...
	html += '<div class="TreeNode" nowrap>'+this.drawIndent(ID)+
				'<a id="node'+ID+'" class="Anchor" title=\''+node.title+'\' href="'+node.Href+'" target="'+this.target+'" onclick ="window.TV['+this.id+'].openFolder('+ID+')" ondblclick="window.TV['+this.id+'].openHandler('+ID+')">'+
				(this.checkbox ? ('<input type="checkbox" id="checkbox'+ID+'"  name="'+this.name+'"' + (node.checkFlag? 'checked':'') + ' ' + (node.disnableFlag? 'disabled':'') + ' value="' + ID + '" onclick="window.TV['+this.id+'].oncheck('+ID+')">') : '')+
